...
### `autofill?: boolean`
Toggle whether the other date should be filled when focus changes. Autofill will trigger on focus change when the start date is after the end date, the end date is before the start date, or one of the fields is empty.