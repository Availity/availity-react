...
| **`onClick`** | Function    | false    |          | Function to run onClick of the link. The first argument passed to `onClick` is the event. The second argument is the processed `url`. |
| **`loadApp`** | String      | false    | `true`   | When `false`, the `url` prop to `AvLink` is _not_ formatted to leverage loadApp.                                                      |
