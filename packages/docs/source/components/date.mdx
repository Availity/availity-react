...
*   **`min`**: limitPropType. Optional. See [limitPropType](https://github.com/Availity/availity-react/blob/master/packages/date/src/utils.js#L45-L49). Used in conjunction with `max` to derive `isOutsideRange` prop from `react-dates`. Dates outside the allowed range will not be clickable in datepicker.
*   **`max`**: limitPropType. Optional. See [limitPropType](https://github.com/Availity/availity-react/blob/master/packages/date/src/utils.js#L45-L49). Used in conjunction with `min` to derive `isOutsideRange` prop from `react-dates`. Dates outside the allowed range will not be clickable in datepicker.
